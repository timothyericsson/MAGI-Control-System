...
"Forward a cURL-style HTTP request via MAGI's relay so you can inspect public source pages or reference endpoints.";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		const full = await getSessionFull(sessionId, userId);
		if (!full.session) {
                                        ...
                                        if (artifactContext) {
                                                systemPrompts.push(
                                                        "Uploaded source context:\n" +
                                                                artifactContext +
                                                                "\n\nUse the uploaded source material as evidence. Reference file paths or document names when relevant, and separate directly supported claims from inference."
                                                );
                                        } else {
                                                systemPrompts.push("Do not invent sources. If the prompt lacks enough evidence, say what is uncertain and what would verify it.");
                                        }
if (liveUrlContext) {
systemPrompts.push(
"Reference URL probe results:\n" +
liveUrlContext +
"\n\nIncorporate only what the probe results actually support. Distinguish observation from interpretation."
);
}
systemPrompts.push(
"You can issue additional HTTP requests using the MAGI curl tool when a public reference URL needs inspection. Summarize what each probe confirmed and avoid unstated assumptions."
);
...
{ role: "user", content: userQuestion },
                                        ...
                                        stageEvents.push(`[${a.name}] draft failed: ${message}`);
                                        await setSessionStatus(sessionId, "error", `${a.name} draft failed`);
                                        throw new Error(`${a.name} draft failed: ${message}`);
                                }
                                ...
                                if (!content) {
                                        stageEvents.push(`[${a.name}] draft failed: empty response`);
                                        await setSessionStatus(sessionId, "error", `${a.name} draft returned empty response`);
                                        throw new Error(`${a.name} draft returned empty response`);
                                }
...
httpRequestCount: chatResult?.httpRequestCount ?? 0,
                                ...
                                stageEvents.push(`[${a.name}] draft stored as #${message.id} via ${actualProvider}`);
                        }
                        const refreshedState = await getSessionFull(sessionId, userId);
                        const proposals = refreshedState.messages.filter((m) => m.role === "agent_proposal");
                        stageEvents.push(`Total drafts recorded: ${proposals.length}`);
                ...
                // Critique step removed: proceed directly from drafts to cross-checking
                        ...
                        await Promise.all(
                                agents.flatMap((a) => {
                                        ...
                                        if (targetableProposals.length === 0) {
                                                stageEvents.push(`[${a.name}] skipped cross-checking: no other drafts available`);
                                        ...
                                        return targetableProposals.map(async (p) => {
                                                ...
                                                try {
                                                        const chatResult = await agentChat(a, keys, [
                                                                { role: "system", content: `You are ${a.name}. Evaluate the candidate answer's factual grounding, uncertainty handling, reasoning quality, completeness, and risk of hallucination. Provide a detailed rationale of at least 3 sentences, referencing specifics. Reply ONLY with a JSON object: {"score": 0-100, "reason": "detailed rationale"}.` },
                                                        ...
                                                        if (parsed) {
                                                                ...
                                                                fallbackUsed = true;
                                                                stageEvents.push(`[${a.name}] score JSON parse fallback for draft #${p.id}: unable to parse structured score`);
                                                                score = Math.max(30, Math.min(90, Math.round(Math.sqrt(p.content.length))));
                                                                rationale = `${a.name} heuristic score`;
                                                        }
                                                } catch (err: any) {
                                                        fallbackUsed = true;
                                                        stageEvents.push(`[${a.name}] score fallback for draft #${p.id}: ${err?.message || "unknown error"}`);
                                                        score = Math.max(30, Math.min(90, Math.round(Math.sqrt(p.content.length))));
                                                ...
                                                stageEvents.push(`[${a.name}] scored draft #${p.id} = ${score}${fallbackUsed ? " (fallback)" : ""}`);
                                                return voteRecord;
                                        });
                                })
                        );
                        const refreshed = await getSessionFull(sessionId, userId);
                ...
                if (step === "consensus") {
                        const stageEvents: string[] = [];
                        const fresh = await getSessionFull(sessionId, userId);
                        ...
                        if (best) {
                                ...
                                stageEvents.push(
                                        `Consensus selected draft #${best.msg.id} (score ${best.score}) as message #${consensusMsg.id}`
                                );
                                await upsertConsensus({ sessionId, finalMessageId, summary: best.msg.content });
                                await setSessionStatus(sessionId, "consensus");
                        } else {
                                stageEvents.push("Consensus failed: no drafts available.");
                                await setSessionStatus(sessionId, "error", "No drafts available for consensus");
                        }
                        const refreshed = await getSessionFull(sessionId, userId);
