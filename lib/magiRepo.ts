...
export async function getSessionFull(sessionId: string, userId?: string): Promise<{
	...
	const supabase = getSupabaseServer();
	let sessionQuery = supabase.from("magi_sessions").select("*").eq("id", sessionId);
	if (userId) {
		sessionQuery = sessionQuery.eq("user_id", userId);
	}
	const [{ data: session, error: sessionError }, agents] = await Promise.all([
	...
	const [{ data: messages }, { data: votes }, { data: consensus }] = await Promise.all([
