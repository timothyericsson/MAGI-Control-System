...
export default function MagiConsensusControl() {
        ...
        const refreshArtifactStatus = useCallback(
                async (artifactId: string): Promise<ArtifactApiResponse> => {
                        ...
                        const res = await fetch(`/api/uploads/${artifactId}`, {
                                cache: "no-store",
        ...
        const handleArtifactUpload = useCallback(
                ...
                [getAuth, question, waitForArtifactReady]
        );
	...
	const loadSessions = useCallback(async () => {
		...
		try {
                        const auth = await getAuth();
			const res = await fetch("/api/magi/sessions", { cache: "no-store", headers: auth.headers });
			const json = await res.json();
	...
	useEffect(() => {
        ...
        const clearHistory = useCallback(async () => {
                ...
                try {
                        const auth = await getAuth();
                        const res = await fetch("/api/magi/sessions", {
                                method: "DELETE",
                                headers: auth.headers,
                        });
                        const json = await res.json();
        ...
        const fetchFull = useCallback(async (sessionId: string) => {
                const auth = await getAuth();
                const res = await fetch(`/api/magi/session/${sessionId}?t=${Date.now()}`, {
                        cache: "no-store",
                        headers: auth.headers,
                });
...
return parts.join(" • ");
        ...
        const liveUrlStatusMessage = useMemo(() => {
                ...
                return `Reference requests will target ${normalizedLiveUrl}`;
        ...
        const stageLabels: { key: "proposing" | "voting" | "finalizing"; label: string; description: string }[] = [
                { key: "proposing", label: "Drafts", description: "Each core answers independently from the available evidence." },
                { key: "voting", label: "Cross-check", description: "The cores score each draft for grounding and hallucination risk." },
                { key: "finalizing", label: "Answer", description: "The strongest answer is surfaced as the consensus result." },
        ];
				...
				<input ref={fileInputRef} type="file" accept=".zip" className="hidden" onChange={onFileInputChange} />
                                <div className="border border-white/10 bg-white/5 rounded-md p-3 mb-4">
                                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                                <div>
                                                        <label className="ui-text text-sm text-white/70 block">Source bundle (.zip)</label>
                                                        <p className="ui-text text-xs text-white/50">
                                                                Optional: attach zipped notes, documents, or source files for evidence grounding. Max {MAX_UPLOAD_MB}MB.
                                                        </p>
                                                </div>
                                                <div className="flex flex-col w-full gap-2 md:w-auto md:flex-row">
                                                        ...
                                                        >
                                                                {artifact ? "Replace sources" : "Select .zip"}
                                                        </button>
                                                        {artifact && (
                                                                <button
                                                                        type="button"
...
Token budget: {artifactTokenSummary}
                                                ...
                                                <p className="ui-text text-xs text-white/50 mt-2">
                                                        No source bundle attached. MAGI will answer from the question and any reference URL.
                                                </p>
                                        )}
                                ...
                                <div className="border border-white/10 bg-white/5 rounded-md p-3 mb-4">
                                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                                <div>
                                                        <label className="ui-text text-sm text-white/70 block">Reference URL</label>
                                                        <p className="ui-text text-xs text-white/50">
                                                                Optional: allow MAGI to inspect a public page or endpoint as supporting source material.
                                                        </p>
                                                </div>
                                                {liveUrl && (
                                                        <button
                                        ...
                                        {!liveUrl && (
                                                <p className="ui-text text-xs text-white/50 mt-2">
                                                        No reference URL provided. MAGI will rely on your question and any uploaded sources.
                                                </p>
                                        )}
                                </div>
                                ...
                                <textarea
                                        ...
                                        placeholder="e.g., What are the strongest arguments for and against this claim, and what evidence would change the answer?"
                                />
                                        ...
                                        {stageLabels.map((stage) => {
                                                ...
                                                return (
                                                        <div key={stage.key} className="bg-white/10 border border-white/10 rounded p-3">
                                                                ...
                                                                {/* Drafts quick summary + modal trigger */}
                                                                {stage.key === "proposing" && (
                                                                        <div className="mt-3">
                                                                                <div className="flex items-center justify-between">
                                                                ...
                                                                {/* Cross-check quick summary + modal trigger */}
                                                                {stage.key === "voting" && (
                                                                        <div className="mt-3">
                                                                                <div className="flex items-center justify-between">
                                                                                        <span className="ui-text text-[11px] text-white/50">{sortedVotes.length} checks tallied</span>
                                                                                        <button
                                                                                                type="button"
                                                        ...
                                                        {historyDetailLoading ? (
                                                                ...
                                                                <div className="space-y-4">
                                                                        ...
                                                                        {historyDetail.liveUrl && (
                                                                                <div className="bg-black border border-white/10 rounded p-3">
                                                                                        <div className="title-text text-sm font-semibold text-white/80">Reference URL</div>
                                                                                        <a
                                                                        ...
                                                                        <div className="bg-black border border-white/10 rounded p-3">
                                                                                <div className="title-text text-sm font-semibold text-white/80">Final Answer</div>
                                                                                {historyDetail.finalMessage ? (
                                                                                        <>
                                                                                                <div className="ui-text text-[11px] text-white/50">
                                                                                                        #{historyDetail.finalMessage.id}
                                                                                                        {historyDetail.derived && " · derived from top-scoring draft"}
                                                                                                </div>
                                                                                                <div className="ui-text text-sm text-white/80 whitespace-pre-wrap mt-2">
                                                                                        ...
                                                                                        <div className="ui-text text-sm text-white/60 mt-2">No final answer recorded.</div>
                                                                                )}
                                                                        </div>
                                                                </div>
                                                        ) : (
                        ...
                        {showProposalsModal && (
                                <div className="fixed inset-0 z-50">
                                        ...
                                        <div className="relative z-10 mx-auto my-8 w-[95vw] max-w-5xl bg-black border border-white/15 rounded-lg shadow-xl">
                                                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                                                        <div className="title-text text-base font-semibold text-white/90">Candidate Answers</div>
                                                        <button
                                                                                ...
                                                                                <div key={proposal.id} className="bg-black border border-white/10 rounded p-3">
                                                                                        ...
                                                                                        <div className="ui-text text-sm text-white/80 whitespace-pre-wrap mt-2">
                                                                                                {proposal.content}
                                                                                                                ...
                                                                                                                <div className="ui-text text-sm text-white/50">No draft answer recorded.</div>
                                                                                                        )}
                                                                                                </div>
                                                                                        </div>
                                                                                );
                        ...
                        {showVotesModal && (
                                <div className="fixed inset-0 z-50">
                                        ...
                                        <div className="relative z-10 mx-auto my-8 w-[95vw] max-w-5xl bg-black border border-white/15 rounded-lg shadow-xl">
                                                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                                                        <div className="title-text text-base font-semibold text-white/90">Cross-check Ledger</div>
                                                        <button
                                                ...
                                                <div className="p-4 overflow-x-auto max-h-[80vh] overflow-y-auto no-scrollbar">
                                                        {sortedVotes.length > 0 ? (
                                                                <table className="min-w-full text-left">
                                                                        <thead>
                                                                                <tr className="ui-text text-[11px] uppercase tracking-widest text-white/40">
                                                                                        ...
                                                                                        <th className="py-2 pr-4 font-normal">Draft</th>
                                                                                                                                        ...
                                                                                                                                        {target ? (
                                                                                                                                                <div className="group relative inline-block">
                                                                                                                                                        ...
                                                                                                                                                        <div className="absolute left-0 top-full mt-1 w-[28rem] max-w-[80vw] bg-black border border-white/20 rounded p-2 shadow-xl z-50 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                                                                                                                                                                <div className="ui-text text-[11px] text-white/50 mb-1">Draft #{target.id}</div>
                                                                ...
                                                                <div className="ui-text text-sm text-white/50">No checks have been submitted.</div>
                                                        )}
                                                </div>
                                        </div>
                                </div>
					...
					<div className="absolute top-0 right-0 h-full w-72 bg-black/50 border-l border-white/15 p-3 overflow-y-auto no-scrollbar backdrop-blur-sm">
						...
						{loadingSessions ? (
							...
							<ul className="space-y-2">
                                ...
                                {/* Cross-check ledger moved inline into the Cross-check stage card */}
                                ...
                                <div className="relative">
                                        ...
                                        <summary className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 cursor-pointer list-none">
                                                <div>
                                                        <h3 className="title-text text-base font-semibold text-white/90">Final Answer</h3>
                                                        <p className="ui-text text-sm text-white/60">
                                                                The selected answer surfaced by the MAGI cores.
                                                        </p>
                                                </div>
                                                <span className="ui-text text-xs text-white/50">
                                        ...
                                        {consensusMessage ? (
                                                                ...
                                                                <div className="ui-text text-sm text-white/70">
                                                                        ...
                                                                        {consensusSourceProposalId ? (
                                                                                <span className="text-white">
                                                                                        draft #{consensusSourceProposalId}
                                                                                </span>
                                                                        ) : (
                                                                                <span className="text-white">the top-scoring draft</span>
                                                                        )}
                                                                </div>
                                                ...
                                                <div className="ui-text text-sm text-white/50 mt-4">
                                                        The answer will appear here once the council finishes deliberating.
                                                </div>
                                        )}
                                        </details>
                                </div>
